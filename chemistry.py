...
chemical_mass = float(input("Please enter the mass in grams:"))
...
def main():
    ...
    key_quantity = parse_formula(chemical_formula, table)

    list = parse_formula(chemical_formula, table)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass (key_quantity, table)
    number_moles = chemical_mass / molar_mass

    get_formula_name(chemical_formula, known_molecules_dict)
    
    sum_protons(list,table)
    sum = sum_protons(list, table)
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for element in symbol_quantity_list:
        ...
        symble =element[SYMBOL_INDEX] 
        number = element[QUANTITY_INDEX]

        element = periodic_table_dict[symble]

        mass = element[ATOMIC_MASS_INDEX]
        total_mass +=mass*number
